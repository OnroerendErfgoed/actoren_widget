...
require(['actorwidget/app/ActorWidget', 'dojo/store/Observable', 'dojo/store/JsonRest', 'dojo/domReady!', '../util/TrackableRestStore'],
  function (ActorWidget, Observable, JsonRest, TrackableRestStore) {
    // const baseUrl= 'http://localhost:6565/';
    const baseUrl = 'https://dev-actoren.onroerenderfgoed.be';
    const actorenUrl = `${baseUrl}/actoren/`;
    const typeLists = {
      emailTypes: [{'naam': 'thuis', 'id': 1}, {'naam': 'werk', 'id': 2}], // get <actorenHost>/email_types
      telephoneTypes: [{'naam': 'thuis', 'id': 1}, {'naam': 'werk', 'id': 2}, {
        'naam': 'mobiel',
        'id': 3
      }, {'naam': 'fax thuis', 'id': 4}, {'naam': 'fax werk', 'id': 5}], // get <actorenHost>/telephone_types
    ...
    const getSsoToken = async () => {
      return 'eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICI4YktMaGdDMUZiTVd5ZFIxSkluQk54bUl' +
    ...
    const getFullActorStore = async () => new TrackableRestStore({
      target: actorenUrl, headers: {
        'Authorization': 'Bearer ' + await getSsoToken(),
        'X-Requested-With': '',
        'Accept': 'application/json'
      }, sortParam: 'sort', useRangeHeaders: true
    });

    const createWidget = async () => {
      const actorStore = await getFullActorStore();

      console.log(await getSsoToken());

      const actorWidget = new ActorWidget({
        actorStore,
        canCreateActor: true,
        canEditActor: true,
